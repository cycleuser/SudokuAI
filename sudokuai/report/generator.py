...
def generate_evaluation_report(result: EvaluationResult, version: str = "1.0.0") -> str:
    ...
    for difficulty, stats in result.results_by_difficulty.items():
        difficulty_results = [
            r for r in result.play_results if r.difficulty == difficulty
        ]
        avg_moves = (
            sum(r.total_moves for r in difficulty_results) / len(difficulty_results)
            if difficulty_results
            else 0
        )
        avg_invalid = (
            sum(r.invalid_moves for r in difficulty_results) / len(difficulty_results)
            if difficulty_results
            else 0
        )
    ...
    avg_moves_per_game = (
        total_moves / result.total_games if result.total_games > 0 else 0
    )

    difficulties_tested = ", ".join(result.results_by_difficulty.keys())
    games_per_difficulty = (
        list(result.results_by_difficulty.values())[0]["total"]
...
def generate_comparison_report(
    ...
    comparison_rows = []
    all_difficulties = set(result1.results_by_difficulty.keys()) | set(
        result2.results_by_difficulty.keys()
    )
